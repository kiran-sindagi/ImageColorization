...
import gc # Import the garbage collector module at the top of your file

def colorizer(img):
    # ... (initial code is the same)
    scaled = img.astype("float32") / 255.0
    lab = cv2.cvtColor(scaled, cv2.COLOR_RGB2LAB)
    del scaled # Clean up the large float array
    ...
    L = cv2.split(lab)[0]
    del lab # Clean up the second large array
...
MAX_DIMENSION = 1024 # Max width or height of 1024 pixels
...
def index():
    if request.method == "POST":
        ...
        # --- RESIZE THE IMAGE ---
        # If the image is larger than our max dimension, shrink it down.
